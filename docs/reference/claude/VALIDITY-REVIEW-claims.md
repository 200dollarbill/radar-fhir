...
Continuation of [VALIDITY-REVIEW.md](VALIDITY-REVIEW.md) (systemic issues S1–S28, the Observation / Patient / Encounter / Device-related claims, summary and consequences live there). Same columns, continuous numbering; `S#` references point to the systemic table in the entry file.
...
| 258 | Write calls send `Content-Type: application/json` | fhir-rest-api.md: MIME `application/fhir+json` SHALL be used; generic `application/json` in Accept → server SHOULD answer JSON | satusehat-specific constraint | Generic JSON type on requests is tolerated, not the R4 canonical type; server must accept both. |
...
| 288 | HL7 terminology systems `http://terminology.hl7.org/CodeSystem/...` for status/category value sets | fhir-condition.md con-4/con-5 and fhir-observation.md print that host for condition-clinical, condition-ver-status, data-absent-reason, v3-ObservationInterpretation | confirmed | Other members of the list not printed on captured pages. |
...
| 308 | 46 supported resources incl. `BillingStatus`, `ChargeItemResponse` | fhir-workflow.md resource lists; no such R4 resources appear on any captured page | conflict | Resource names that do not exist in R4 (S20). |
