...
source_url: https://satusehat.kemkes.go.id/platform/docs/id/api-catalogue/integrations/apis/observation/
group: satusehat
fhir_version: R4
fetched_at: '2026-09-11T13:21:13Z'
sha256: 9ebec461156321f76a11bfc317a5758722304330f56fb3578080195dfe8ae3c4
---
# Observation
...
| `encounter` | `string` | Berisi ID dari *Encounter* (kunjungan) yang akan dicari.  Contoh: `4f735a03-128b-464d-bf91-e6eacdf1c38f`. |

##### 2. Pencarian Berdasarkan ID Subjek (Pasien) dan ID *ServiceRequest*

| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `?subject` | `string` | Parameter ini **WAJIB** **ada bila melakukan pencarian data dengan ID subjek (pasien) dan ID *ServiceRequest***. Berisi ID dari subjek (pasien) yang akan dicari.  Contoh: `100000000001`. |
| `?based-on` | `string` | Parameter ini **WAJIB** **ada bila melakukan pencarian data dengan ID subjek (pasien) dan ID *ServiceRequest***. Berisi ID dari *ServiceRequest* yang akan dicari.  Contoh: `6694e8c8-052a-4ea6-8072-157b6d47ca08`. |
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource Observation yang akan dilihat detailnya. |
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource Observation yang ingin didapatkan datanya.
6. Klik tombol **Send**.
7. Hasil *response* akan ditampilkan di bagian **Response**.
...
Dari hasil *response* ini, **PERLU** disimpan nilai UUID yang didapat dari properti `id`, di mana nilai tersebut kemungkinan akan digunakan dalam proses lainnya yang terkait resource Observation ini.
...
5. Pada tab **Body**:
   ...
   3. terakhir masukkan *resource* JSON dari *Observation* yang akan diproses ke kotak masukkan di bawah pilihan tadi. Contoh:
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource Observation yang akan dilakukan proses pembaruan data (*update*). |
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource Observation yang akan diperbarui (*update*).
6. Pada tab **Body**:
...
- `<operasi>`, saat ini hanya tersedia operasi `replace` saja, yaitu untuk menganti nilai properti/elemen dari *resource* Observation dengan ID terkait.
- `<element-path>`, nama properti/element dari *resource* Observation dengan ID terkait yang akan diganti nilainya, dengan format `/<path>/<to>/<element>`. Simbol `/` sebagai pemisah dari nama properti/element yang dimaksud.

  Contoh, misalkan pada *resource* Observation ini ada properti/element dengan nama `language`, yang dalam bentuk path FHIR `Observation.language`, maka penulisan nilai `<element-path>` ini adalah `/language`.
