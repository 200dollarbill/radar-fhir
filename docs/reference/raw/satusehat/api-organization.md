...
source_url: https://satusehat.kemkes.go.id/platform/docs/id/api-catalogue/onboardings/apis/organization/
group: satusehat
fhir_version: R4
fetched_at: '2026-09-11T13:21:07Z'
sha256: 8035d59d717729873b5800b93cac0124cbd0413693407389def8be9faed38ead
---
# Organization
...
##### 1. Pencarian Berdasarkan Nama Organisasi

| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `?name` | `string` | Parameter ini **WAJIB** **ada bila melakukan pencarian data dengan nama organisasi**. Berisi nama, baik sebagian atau lengkap, dari organisasi yang akan dicari.  Contoh: `paramarta`. |

##### 2. Pencarian Berdasarkan ID Induk dari Organisasi

| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `?partof` | `string` | Parameter ini **WAJIB** **ada bila melakukan pencarian data dengan ID induk organisasi**. Berisi ID Induk dari organisasi yang akan dicari.  Contoh: `10000004`. |
...
Bila resource Organization dengan ID terkait berhasil ditemukan atau tersedia, maka akan mengembalikan data dari resource Organization yang tersimpan di ekosistem SATUSEHAT.
...
```
{
  ...
  "entry": [
    {
      "fullUrl": "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Organization/38d2fd4d-1402-4e5f-8f09-618fca5ce313",
      "resource": {
        "resourceType": "Organization",
        "id": "38d2fd4d-1402-4e5f-8f09-618fca5ce313",
        //data.terkait.resource.Organization
      },
      "search": {
        "mode": "match"
      }
...
curl --insecure --location ^
  ...
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Organization?partof=10000004"
```
...
Fungsi dari ReST API ini adalah untuk mendapatkan data terkait resource Organization yang tersedia di ekosistem SATUSEHAT. Untuk mendapatkan data yang dimaksud, nilai ID dari resource Organization tersebut **PERLU** diketahui dan disediakan sebagai parameternya.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Organization/:id
```
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource Organization yang akan dilihat detailnya. |
...
curl --insecure --location ^
  --header "Authorization: Bearer <access-token>" ^
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Organization/38d2fd4d-1402-4e5f-8f09-618fca5ce313"
```
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource Organization yang ingin didapatkan datanya.
6. Klik tombol **Send**.
7. Hasil *response* akan ditampilkan di bagian **Response**.

## Penambahan Data

Fungsi dari ReST API ini adalah untuk melakukan penambahan data terkait resource Organization ke dalam ekosistem SATUSEHAT.
...
Dari hasil *response* ini, **PERLU** disimpan nilai UUID yang didapat dari properti `id`, di mana nilai tersebut kemungkinan akan digunakan dalam proses lainnya yang terkait resource Organization ini.
...
5. Pada tab **Body**:
   ...
   3. terakhir masukkan *resource* JSON dari *Organization* yang akan diproses ke kotak masukkan di bawah pilihan tadi. Contoh:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan data terkait resource Organization ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan (*update*) data, **PERLU** ID dari resource Organization yang akan diubah.
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource Organization yang akan dilakukan proses pembaruan data (*update*). |
...
Bila proses pembaruan data berhasil maka akan mengembalikan *payload* dari resource Organization yang sebelumnya telah dikirim.
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource Organization yang akan diperbarui (*update*).
6. Pada tab **Body**:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan sebagian dari data terkait resource Organization ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan sebagian (*patching*) data, **PERLU** ID dari resource Organization yang akan diubah dan juga nama/ID elemen dari Organization yang akan dilakukan perubahan.
...
- `<operasi>`, saat ini hanya tersedia operasi `replace` saja, yaitu untuk menganti nilai properti/elemen dari *resource* Organization dengan ID terkait.
- `<element-path>`, nama properti/element dari *resource* Organization dengan ID terkait yang akan diganti nilainya, dengan format `/<path>/<to>/<element>`. Simbol `/` sebagai pemisah dari nama properti/element yang dimaksud.

  Contoh, misalkan pada *resource* Organization ini ada properti/element dengan nama `language`, yang dalam bentuk path FHIR `Organization.language`, maka penulisan nilai `<element-path>` ini adalah `/language`.
