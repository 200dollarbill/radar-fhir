...
id: api-encounter
title: API Encounter
source_url: https://satusehat.kemkes.go.id/platform/docs/id/api-catalogue/integrations/apis/encounter/
group: satusehat
fhir_version: R4
fetched_at: '2026-09-11T13:21:12Z'
sha256: 21318ad6dc327f318015bd9034052e115eda3540a6812807ca3b7235f2ae1ec0
---
# Encounter

## Pencarian Data

Fungsi dari ReST API ini adalah untuk mencari data terkait resource Encounter yang tersedia di ekosistem SATUSEHAT dengan parameter-parameter tertentu.
...
Bila resource Encounter dengan ID terkait berhasil ditemukan atau tersedia, maka akan mengembalikan data dari resource Encounter yang tersimpan di ekosistem SATUSEHAT.
...
```
{
  ...
  "entry": [
    {
      "fullUrl": "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Encounter/4f735a03-128b-464d-bf91-e6eacdf1c38f",
      "resource": {
        "resourceType": "Encounter",
        "id": "4f735a03-128b-464d-bf91-e6eacdf1c38f",
        //data.terkait.resource.Encounter
      },
      "search": {
        "mode": "match"
      }
...
curl --insecure --location ^
  ...
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Encounter?subject=100000000001"
```
...
Fungsi dari ReST API ini adalah untuk mendapatkan data terkait resource Encounter yang tersedia di ekosistem SATUSEHAT. Untuk mendapatkan data yang dimaksud, nilai ID dari resource Encounter tersebut **PERLU** diketahui dan disediakan sebagai parameternya.
...
```
https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Encounter/:id
```
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource Encounter yang akan dilihat detailnya. |
...
curl --insecure --location ^
  --header "Authorization: Bearer <access-token>" ^
  --request GET ^
  "https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1/Encounter/4f735a03-128b-464d-bf91-e6eacdf1c38f"
```
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource Encounter yang ingin didapatkan datanya.
6. Klik tombol **Send**.
7. Hasil *response* akan ditampilkan di bagian **Response**.

## Penambahan Data

Fungsi dari ReST API ini adalah untuk melakukan penambahan data terkait resource Encounter ke dalam ekosistem SATUSEHAT.
...
Dari hasil *response* ini, **PERLU** disimpan nilai UUID yang didapat dari properti `id`, di mana nilai tersebut kemungkinan akan digunakan dalam proses lainnya yang terkait resource Encounter ini.
...
5. Pada tab **Body**:
   ...
   3. terakhir masukkan *resource* JSON dari *Encounter* yang akan diproses ke kotak masukkan di bawah pilihan tadi. Contoh:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan data terkait resource Encounter ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan (*update*) data, **PERLU** ID dari resource Encounter yang akan diubah.
...
| Nama Parameter | Tipe Data | Keterangan |
| --- | --- | --- |
| `*:id` | `uuid` | ID referensi dari resource Encounter yang akan dilakukan proses pembaruan data (*update*). |
...
Bila proses pembaruan data berhasil maka akan mengembalikan *payload* dari resource Encounter yang sebelumnya telah dikirim.
...
5. Pada tab **Params**, di bagian **Path Variables**:
   ...
   1. Isi nilai parameter `id` dengan ID dari resource Encounter yang akan diperbarui (*update*).
6. Pada tab **Body**:
...
Fungsi dari ReST API ini adalah untuk melakukan perubahan sebagian dari data terkait resource Encounter ke dalam ekosistem SATUSEHAT, yang sebelumnya sudah ditambahkan dan tersedia di dalam ekosistem SATUSEHAT. Untuk melakukan perubahan sebagian (*patching*) data, **PERLU** ID dari resource Encounter yang akan diubah dan juga nama/ID elemen dari Encounter yang akan dilakukan perubahan.
...
- `<operasi>`, saat ini hanya tersedia operasi `replace` saja, yaitu untuk menganti nilai properti/elemen dari *resource* Encounter dengan ID terkait.
- `<element-path>`, nama properti/element dari *resource* Encounter dengan ID terkait yang akan diganti nilainya, dengan format `/<path>/<to>/<element>`. Simbol `/` sebagai pemisah dari nama properti/element yang dimaksud.

  Contoh, misalkan pada *resource* Encounter ini ada properti/element dengan nama `language`, yang dalam bentuk path FHIR `Encounter.language`, maka penulisan nilai `<element-path>` ini adalah `/language`.
