---
id: secpriv-module
title: Security and Privacy Module
source_url: https://hl7.org/fhir/R4/secpriv-module.html
group: fhir-r4
fhir_version: R4
fetched_at: '2026-09-11T13:22:16Z'
sha256: 03393a645a20f763a5897cccfc88d54ebc7e6df3c34894325fd11ea9a5a2619f
---
This page is part of the FHIR Specification (v4.0.1: R4 - Mixed [Normative](https://confluence.hl7.org/display/HL7/HL7+Balloting "Normative Standard") and [STU](https://confluence.hl7.org/display/HL7/HL7+Balloting "Standard for Trial-Use")) in it's permanent home (it will always be available at this URL). The current version which supercedes this version is [5.0.0](http://hl7.org/fhir/index.html). For a full list of available versions, see the [Directory of published versions ![](external.png)](http://hl7.org/fhir/directory.html). Page versions: [R5](http://hl7.org/fhir/R5/secpriv-module.html) [R4B](http://hl7.org/fhir/R4B/secpriv-module.html) **R4** [R3](http://hl7.org/fhir/STU3/secpriv-module.html)

|  |  |
| --- | --- |
| Work Group [FHIR Infrastructure](http://www.hl7.org/Special/committees/fiwg/index.cfm)  & [Security](http://www.hl7.org/Special/committees/secure/index.cfm) | [Standards Status](versions.html#std-process): [Informative](versions.html#std-process) |
...
| - [Consent](consent.html) - [Provenance](provenance.html) - [Audit Event](auditevent.html) | - [Signature](datatypes.html#signature) | - [Security Principles](security.html) - [Security Labels](security-labels.html) - [Signatures](signatures.html) |
...
For general security considerations and principles, see [Security](security.html).
...
See  [NIST Mobile Device Security ![](external.png)](https://nccoe.nist.gov/projects/building_blocks/mobile_device_security)
and [OWASP Mobile Security ![](external.png)](https://www.owasp.org/index.php/Mobile_Top_10_2016-Top_10).
...
*Use case:* A FHIR server should ensure that API access is allowed for
authorized requests and denied for unauthorized requests.

*Approach:* Authorization details can vary according to local policy,
...
see [Security: Authentication](security.html#authentication) as an
...
For further details, see [Security: Authorization and Access Control](security.html#binding).
...
*Approach:* Enforcement is by local enforcement methods. Note that community efforts are underway to explore a variety of
...
*Use case:* "Access to protected Resources are enabled though user Role-Based,
Context-Based, and/or Attribute-Based Access Control."

*Approach:* Ensure that the level of assurance for identity proofing reflects the appropriate risk, given the issued party's exposure to health information. Users should be identified and should have their Functional and/or Structural role
declared when these roles are related to the functionality the user is interacting with. Roles should
be conveyed using standard codes from [Security Role Vocabulary](valueset-security-role-type.html).

A purpose of use should be asserted for each requested action on a Resource. Purpose of use
should be conveyed using standard codes from [Purpose of Use Vocabulary](v3/PurposeOfUse/vs.html).
...
For details, see
[Security: HCS vocabulary](security-labels.html#hcs).

#### 6.0.5.3 Audit Logging

*Use case:* "A FHIR server should keep a complete, tamper-proof log of
all API access and other security- and privacy-relevant events".

*Approach:* FHIR provides an AuditEvent resource suitable for
...
For details, see
[Security: Audit](security.html#audit).

#### 6.0.5.4 Accounting of Disclosures and/or Access Report
...
*Approach:* FHIR provides a Consent resource suitable for
...
and supports digital signatures through use of [Provenance](provenance.html).
The Privacy Consent has the ability to include some simple FHIR centric base and exception rules.

All uses of FHIR Resources would be security/privacy relevant and thus should be recorded in an [AuditEvent](auditevent.html).
The data access that qualifies as a Disclosure should additionally be recorded as a Disclosure, see [Disclosure Audit Event Example](auditevent-example-disclosure.html).

For Privacy Consent guidance and examples, see
[Consent Resource](consent.html).

#### 6.0.5.6 Provenance

*Use case:* "All FHIR Resources should be capable of having the Provenance fully described."

*Approach:* FHIR provides the Provenance resource suitable for
...
Generally, Read operations would be recorded using [AuditEvent](auditevent.html).
...
This [overlap is expected and cross-referenced on the Five Ws pattern](fivews.html).
For details, see
[Provenance Resource](provenance.html).
...
For details, see [\_revinclude](search.html#revinclude).

#### 6.0.5.7 Signature

*Use case:* "Digital Signature is needed to prove authenticity, integrity, and non-repudiation."

*Approach:* FHIR Resources are often parts of Medical Record or are communicated as part of formal Medical Documentation.
...
This functionality is provided through the signature element in [Provenance](provenance.html) Resource.
...
For details, see
[Ramifications of storage/retrieval variations](updates.html)
...
*Trigger:* Based on an Access Control decision that results in a permit with an Obligation to De-Identify, the Results delivered
to the Requesting Client would be de-identified.

*Consideration:* This assumes the system knows the type and intensity of the de-identification algorithm, where
...
*Modifying an element:* The de-identification process may determine that specific elements need to be modified to lower privacy
...
some random value), masking (e.g. encryption), pseudonym (e.g. replace with an alias), etc. [Narrative](narrative.html) and [Attachment](datatypes.html#Attachment) elements present particularly difficult challenges.
...
many different types of ids and identifiers that serve different purposes. Some (`id`s) are the basis for internal
...
*Emphasis:* The .specimen is a direct identifier of a particular specimen; and would be a direct identifier of a particular patient.
...
Further Standards:
[Health: ISO Pseudonymization ![](external.png)](http://www.iso.org/iso/catalogue_detail?csnumber=42807),
[NIST IR 8053 - De-Identification of Personal Information ![](external.png)](http://nvlpubs.nist.gov/nistpubs/ir/2015/NIST.IR.8053.pdf),
[IHE De-Identification Handbook ![](external.png)](http://wiki.ihe.net/index.php/Healthcare_De-Identification_Handbook),
[DICOM (Part 15, Chapter E) ![](external.png)](http://dicom.nema.org/medical/dicom/current/output/html/part15.html#chapter_E)

#### 6.0.5.9 Using FHIR to publish test data
