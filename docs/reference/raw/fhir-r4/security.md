...
group: fhir-r4
fhir_version: R4
fetched_at: '2026-09-11T13:22:14Z'
sha256: 58f61ff28149035d1d608f53def305e4f2066f999a6f1778441f064ab51907a5
---
This page is part of the FHIR Specification (v4.0.1: R4 - Mixed [Normative](https://confluence.hl7.org/display/HL7/HL7+Balloting "Normative Standard") and [STU](https://confluence.hl7.org/display/HL7/HL7+Balloting "Standard for Trial-Use")) in it's permanent home (it will always be available at this URL). The current version which supercedes this version is [5.0.0](http://hl7.org/fhir/index.html). For a full list of available versions, see the [Directory of published versions ![](external.png)](http://hl7.org/fhir/directory.html). Page versions: [R5](http://hl7.org/fhir/R5/security.html) [R4B](http://hl7.org/fhir/R4B/security.html) **R4** [R3](http://hl7.org/fhir/STU3/security.html) [R2](http://hl7.org/fhir/DSTU2/security.html)

# 6.1.0 FHIR Security

|  |  |  |
| --- | --- | --- |
| [FHIR Infrastructure](http://www.hl7.org/Special/committees/fiwg/index.cfm)  Work Group | [Maturity Level](versions.html#maturity): 4 | [Standards Status](versions.html#std-process): [Trial Use](versions.html#std-process) |
...
3. [Authentication](#authentication) - Users/Clients must be authenticated. For web-centric, OAuth is recommended. When using OAuth, a profile of OAuth will be needed. Consider use of [Smart-On-FHIR ![](external.png)](http://docs.smarthealthit.org/) where appropriate.
...
9. Data Management Policies - FHIR defines a set of capabilities to support data exchange.
   ...
   It is the responsibility of implementers to ensure that relevant regulations and other requirements are met
...
the [FHIR email
list ![](external.png)](https://confluence.hl7.org/display/FHIR/Mailing+List+Instructions) for prompt consideration.
...
|  |  |  |  |  |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
|  |  | |  |  | | --- | --- | |  | The consumer that is using a healthcare related system | |  | The client application the user is using (application, mobile app, website, etc.) | |  | The security system (authentication and access control) | |  | The clinical/healthcare repository | |
...
This is normal web-server use of [https](#http).
...
These Resources will often use the [security labels](security-labels.html) to differentiate various confidentiality levels within this broad group of Patient Sensitive data.
Access to these Resources often requires a declared Purpose Of Use.
Access to these Resources is often controlled by a [Privacy Consent](consent.html).
See the section below on [Authorization and Access Control](#binding).

### 6.1.0.2.5 Not classified
...
Please follow the  [HTTP specification Security Considerations section 9 ![](external.png)](https://tools.ietf.org/html/rfc7231#section-9).
The [Service Base URL](http.html#root) will specify whether TLS is required.
...
Please follow IETF Best Current Practice ([BCP 195 ![](external.png)](https://tools.ietf.org/html/bcp195)) - ["Recommendations for Secure Use of Transport Layer Security (TLS) and Datagram Layer Security (DTLS)" ![](external.png)](https://tools.ietf.org/html/bcp195). This recommends TLS 1.2 or higher be used for all production data exchange, limits support for lower versions of TLS, and forbids SSL.
...
enabling [cross-origin resource sharing (CORS) ![](external.png)](https://www.w3.org/TR/cors/) for the [REST operations](http.html).
Consider advice from sources including [Enable-CORS ![](external.png)](http://enable-cors.org/) and [Moesif blog on Guide to CORS Pitfalls ![](external.png)](https://www.moesif.com/blog/technical/cors/Authoritative-Guide-to-CORS-Cross-Origin-Resource-Sharing-for-REST-APIs/).
...
[OpenID Connect ![](external.png)](http://openid.net/connect/) (or other suitable authentication protocol) to verify identity of the end user, where it is necessary that end-users be identified to the application.
It is recommended that [OAuth ![](external.png)](http://oauth.net/) be used to authenticate and/or authorize
the client and user. The [Smart-On-FHIR ![](external.png)](http://docs.smarthealthit.org/) profile
on OAuth is a recommended method for using OAuth.

The [HEART Working Group ![](external.png)](https://openid.net/wg/heart/) has developed a
...
IHE [IUA Profile ![](external.png)](http://wiki.ihe.net/index.php/Internet_User_Authorization) constrains OAuth token attributes to support Healthcare.
...
[IHE Access Control white paper ![](external.png)](http://www.ihe.net/Technical_Framework/upload/IHE_ITI_TF_WhitePaper_AccessControl_2009-09-28.pdf)

Access control constraints may result in data returned in a read or search being
redacted or otherwise restricted. See [Variations between
Submitted data and Retrieved data](updates.html).
...
### 6.1.0.5.2 Access Denied Response Handling
...
Note that if a server allows [PUT to a new location](http.html#upsert),
...
FHIR provides an [AuditEvent](auditevent.html) resource suitable for use by
...
[IHE-ATNA profile ![](external.png)](http://wiki.ihe.net/index.php?title=Audit_Trail_and_Node_Authentication).
...
[with CDA ![](external.png)](http://smartplatforms.org/2014/04/security-vulnerabilities-in-ccda-display/)). For
this reason, the [FHIR narrative can't contain active content](narrative.html#security).
...
- Ensure that any external references to images or anchors (e.g. outside the resource) do not cause the display software to [leak sensitive information in headers ![](external.png)](http://smartplatforms.org/2014/04/security-vulnerabilities-in-ccda-display/)
...
In addition to narrative, [Documents](documents.html) may also contain
